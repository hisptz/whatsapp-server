...
import {ContactType, WhatsappMessage} from "../interfaces";
...
class WhatsappService {
    ...
    * */
    ...
    async getChatIds(contacts: { type: ContactType; number: string }[]): Promise<Awaited<string>[]> {
    ...
    async sendMessage(message: WhatsappMessage) {
        ...
        switch (type) {
            case "image":
                return Promise.all(chatIds.map((chatId) => this.sendImageMessage(chatId, image, text)))
            case "text":
                if (!text) {
                    throw `Please specify text to send`
                }
                return Promise.all(chatIds.map((chatId) => this.sendTextMessage(chatId, text)));
            default:
    ...
    protected async sendImageMessage(chatId: string, imagePath: string, caption?: string) {
